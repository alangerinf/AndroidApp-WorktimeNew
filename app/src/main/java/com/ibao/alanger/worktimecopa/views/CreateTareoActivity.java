...
import com.ibao.alanger.worktimecopa.models.DAO.CentroCosteDAO;
...
import com.ibao.alanger.worktimecopa.models.DAO.LoteDAO;
...
public class CreateTareoActivity extends AppCompatActivity {
    ...
    private void cargarDirecto(){
        ...
        spnFundo.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> adapterView, View view, int i, long l) {
                ...
                Log.d(TAG,"cargando cultivo desde spnFundo.setOnItemSelectedListener ");
                loadSpnCultivos();
            }
            @Override
            public void onNothingSelected(AdapterView<?> adapterView) {
        ...
        if(isFirst && MY_CREATE_MODE.equals(MODE_EDIT)){
            handler.removeCallbacks(runLoadDirectoIsFirst);
            handler.postDelayed(runLoadDirectoIsFirst,300);
        }
    ...
    private void cargarIndirecto(){
        ...
        if(isFirst && MY_CREATE_MODE.equals(MODE_EDIT)){
            handler.removeCallbacks(runLoadIndirectoIsFirst);
            handler.postDelayed(runLoadIndirectoIsFirst,300);
        }
    }
    ...
    private void cargarAsistencia(){
        ...
        if(isFirst && MY_CREATE_MODE.equals(MODE_EDIT)){
            handler.removeCallbacks(runLoadAsistenciaIsFirst);
            handler.postDelayed(runLoadAsistenciaIsFirst,300);
        }
    ...
    private void loadListCultivos() {
        ...
        int pos = spnFundo.getSelectedItemPosition();
        Log.d(TAG,"loadListCultivos "+listFundos.get(pos/*-1*/).getId());
        listCultivos = cultivoDAO.selectByIdFundo(listFundos.get(pos/*-1*/).getId());
